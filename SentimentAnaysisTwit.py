...
vocab = {words:ii for ii,words in enumerate(filtered_words)}
# Reverse of the `vocab` dictionary. The key is word id and value is the word. 
id2vocab = {words:ii for words,ii in enumerate(filtered_words)}
...
class TextClassifier(nn.Module):
    ...
    def forward(self, nn_input, hidden_state):
        ...
        r_output, hidden_state = self.lstm(embeds,hidden_state)
        r_output = r_output[-1,:,:]   
        output = self.dropout(r_output)
        output = self.fc(output)
        logout = self.softmax(output)
        return logout, hidden
...
for epoch in range(epochs):
    ...
    for text_batch, labels in dataloader(
        ...
        steps += 1
        hidden = model.init_hidden(list(labels.shape)[0])
        ...
        if steps % print_every == 0:
            ...
            valid_hidden =  model.init_hidden(list(labels.shape)[0])
            valid_losses = []
            accuracy = []
...
def predict(text, model, vocab):
    ...
    text_input = torch.from_numpy(np.asarray(torch.FloatTensor(tokens).view(-1,1))).to(device)
    # Get the NN output
...
# The model classifies the input to five outputs. The model is using categorial distribution to assign confidential degree to every class for the input.For instance the model is confident by 97.69% that the output is positive and by 0.01% 
