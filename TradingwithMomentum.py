...
def resample_prices(close_prices, freq='M'):
    ...
    """
    resampledClose_prices = close_prices.resample(freq).last()
    
    return resampledClose_prices
...
def compute_log_returns(prices):
    ...
    """
    logP=np.log(prices/prices.shift(1))
...
def get_top_n(prev_returns, top_n):
    ...
    """
    copyofdf = prev_returns.copy()
   
    return copyofdf.apply(lambda currser: currser>=pd.Series.nlargest(currser,top_n).min(),axis=1).astype('int64')
...
# According to the rule, the smaller the p value, the higher rejection of the null hypothesis. The observed p-value is 0.073359. The value is above the 0.05, so the null hypothesis is accepted . One should not conclude this portfilo. The best way to add different options or insturments and test again the null hypothesis
