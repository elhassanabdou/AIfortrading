...
def filter_signals(signal, lookahead_days):
    ...
    """
    #TODO: Implement function
    cpsignal = signal.copy()
    for ticker , ticker_signals in signal.T.iterrows():
        longsignal = ticker_signals.replace(-1,0)
        shortsignal = ticker_signals.replace(1,0)
        longsignal = clear_signals(longsignal,lookahead_days)
        shortsignal = clear_signals(shortsignal,lookahead_days)
        cpsignal[ticker] = longsignal+shortsignal
    return cpsignal
...
# #The 5 days histogram shows positive skewed log normal distribution. This means that there is a positive log return for the 5 days model data. The other 2 histograms shows bumpy tails which usually means there are outliers contributed in calculating the log return.
...
def calculate_kstest(long_short_signal_returns):
    ...
    kptest=[]
    col =[]
    lsargs = (np.mean(long_short_signal_returns['signal_return']), np.std(long_short_signal_returns['signal_return'] , ddof=0))
    for nm, grp in long_short_signal_returns.groupby('ticker'):
        grpval = grp['signal_return'].values
        ks,p = kstest(rvs=grpval,cdf='norm',args=lsargs)
        ktest.append(ks)
        kptest.append(p)
        col.append(nm)
    ks_values = pd.Series(ktest,col)
    p_values= pd.Series(kptest,col)
...
def find_outliers(ks_values, p_values, ks_threshold, pvalue_threshold=0.05):
    ...
    """
   # My very first solution was using for loop to loop over the two series to find out the which index fits both condition and then return series of names
    # I was not convinced with solution and I found the below solution in the dicussion forum I understand it.
    return set(p_values[p_values < pvalue_threshold].index).intersection(ks_values[ks_values > ks_threshold].index)
